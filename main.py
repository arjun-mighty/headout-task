...
def generate_dockerfile(jarfile: str, output_dir: str):
    ...
    # We only need the jar filename, so strip off the parent dirs
    # eg: "webservice/build/lib/testing-0.0.1-SNAPSHOT.jar" -> testing-0.0.1-SNAPSHOT.jar
    jarfile = jarfile.rsplit("/", maxsplit=1)[-1]
...
def main():
    ...
    try:
        infer_proc_out = subprocess.run(f"ls -t1 {target_dir}/build/lib/*.jar | head -n1", shell=True, check=True, capture_output=True, text=True)
